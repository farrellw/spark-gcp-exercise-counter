...
object Workouts {
  ...
  def initAnotherWorkoutDatastore: Option[WorkoutList] = {
    if (workoutList.isEmpty) {
      ...
      val lines = filesource.getLines().map(_.split(",")).toList.sortBy(_.length).map(w => Word(w(0).toLowerCase, valid = true, w.lift(1).map(_.toLowerCase).getOrElse(w(0)).toLowerCase()))
      filesource.close()
